...
When a user runs `sshai copy-id` inside the remote shell, the shim sends a framed request to the Unix socket. The agent forwards it over its dedicated SSH channel; the local dispatcher performs the SFTP operation and returns stdout, stderr, and an exit code synchronously. `sshai --agent codex` and `sshai --agent claude` use the same control path, include the primary remote current working directory, temporarily restore the local terminal, and let the selected local AI CLI inherit it. The local directory from which the outer `sshai` session started remains the local read-write workspace. When the child exits, the client restores raw mode and resumes the existing remote shell. Single-host terminal data remains a transparent byte stream; multi-host mode additionally parses output into an independent VT100 screen for each host so focus changes can restore a snapshot without replaying history.

An interactive target list such as `sshai build,test,prod` connects and opens the first target's PTY synchronously, then starts all secondary SSH connections in the background without delaying first-host input. Background authentication is deliberately non-interactive so password, key-passphrase, keyboard-interactive, and unknown-host prompts cannot steal the raw terminal; SSH Agent/public-key authentication and pre-established host trust continue normally. Each successful target owns an independent PTY, progressive worker, cwd, foreground process, and VT100 screen. `Shift+Left/Right` byte sequences switch the input/output focus among live PTYs, while the older `Ctrl+Shift+Left/Right` and prefix form remain supported. Inactive output updates only its host's virtual screen rather than mixing into the visible terminal. Focus changes restore one fixed-size visible snapshot, terminal resize events update every virtual screen and live PTY, and a normal exit from the active shell shuts down the whole multiplexer.
