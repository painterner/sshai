...
An interactive target list such as `sshai build,test,prod` opens the PTY on the first target. The AI adapters retain the complete ordered target list and inject one independently named MCP server per remote host. Secondary SSH/MCP connections are lazy and start only when the AI CLI starts, preserving shell startup latency. The primary target inherits the remote shell cwd; secondary targets use their explicitly supplied roots or remote home directories.
