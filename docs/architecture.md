...
When a user runs `sshai copy-id`, `sshai get`, or `sshai put` inside the remote shell, the shim sends a framed request to the Unix socket. The agent forwards it over its dedicated SSH channel; the local dispatcher performs the SFTP operation and returns stdout, stderr, and an exit code synchronously. The session's startup files also install path completion for bash, zsh, and fish: remote-side arguments use the shell's own file completion, and local-side arguments call the hidden `sshai --complete-local PREFIX` control command, which lists candidates from the client's working directory. zsh gets a native completion function because `bashcompinit` provides no `compopt`, and fish reads a completions file added to `fish_complete_path`. `get` and `put` resolve the remote path against the requesting shell's reported cwd and the local path against the client's own working directory, then run SFTP over the same authenticated session; the multiplexer answers these built-in commands from a task so a long transfer cannot stall the other panes, and their reply is exempt from the shim's 300-second timeout. `sshai --agent codex` and `sshai --agent claude` use the same control path, include the primary remote current working directory, temporarily restore the local terminal, and let the selected local AI CLI inherit it. The local directory from which the outer `sshai` session started remains the local read-write workspace. When the child exits, the client restores raw mode and resumes the existing remote shell. Every interactive target session, single-host included, runs through the multiplexer so that the local shell pane exists everywhere; output is parsed into an independent VT100 screen per pane so focus changes can restore a snapshot without replaying history.

An interactive target list such as `sshai build,test,prod` connects and opens the first target's PTY synchronously, then starts all secondary SSH connections in the background without delaying first-host input. Background authentication is deliberately non-interactive so password, key-passphrase, keyboard-interactive, and unknown-host prompts cannot steal the raw terminal; SSH Agent/public-key authentication and pre-established host trust continue normally. Each successful target owns an independent PTY, progressive worker, cwd, foreground process, and VT100 screen. The multiplexer always owns one extra pane: a local login shell on its own PTY (`openpty` plus `setsid`), ordered first so it matches the numbering of `sshai hosts`. `Shift+Left/Right` byte sequences switch the input/output focus among the live PTYs, while the older `Ctrl+Shift+Left/Right` and prefix forms remain supported; `Ctrl+Left/Right` are deliberately left to the shell's word motion. The local pane runs no remote worker, so session control commands stay with the SSH panes; a local shell that exits returns focus to the host it was opened from, keeps its ring slot, and is restarted the next time the switcher selects it. Inactive output updates only its host's virtual screen rather than mixing into the visible terminal. Because each pane's connection list is text written into its screen once, the multiplexer appends the finished list to every pane as soon as no target is still connecting, and asks the pane in view to repaint so its prompt survives the injected block; a host that closes later reports one line instead. Focus changes restore one fixed-size visible snapshot, terminal resize events update every virtual screen and live PTY, and a normal exit from the active shell shuts down the whole multiplexer.

`sshai sync` keeps a local and a remote directory equal in both directions. A pass scans both sides, resolves a BLAKE3 digest for every file — remotely through the worker's `hash` operation, so no content crosses the wire to compare — and reconciles the two snapshots against the ancestor state stored per endpoint pair under `$XDG_STATE_HOME/sshai/sync/`. Reconciliation is a pure function over the three snapshots: a change on one side propagates, a change on both is a conflict resolved by policy or reported, and a first pass (no ancestor) never deletes. Only the resulting copies move, over SFTP. Directory removal is never recursive: children are deleted first and the directory is then removed on its own, so ignored content cannot be destroyed, and a directory the other side deleted while it still holds ignored, new, or changed content is kept and reported. Digests are cached per side under the size and timestamp a scan already sees, so a steady-state pass hashes nothing; a failed action keeps its path out of the new ancestor so the next pass retries it.

The local pane follows the same exit rule as a remote pane: an `exit` or EOF in the active pane closes the complete multiplexer and all SSH sessions. It does not fall back to another pane and require a second `exit`.
